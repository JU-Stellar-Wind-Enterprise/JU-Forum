...
export default async function PostDetailPage({
  ...
  return (
    <main className="mx-auto max-w-3xl p-8">
      ...
      <p className="mt-2 text-sm text-zinc-500">By {post.author.name}</p>
      <p className="mt-8 whitespace-pre-wrap">{post.content}</p>
      <VoteButtons
        postId={post.id}
        {...votes}
        disabled={post.isLocked || post.isArchived || user.status !== "ACTIVE"}
      />
    </main>
  );
}